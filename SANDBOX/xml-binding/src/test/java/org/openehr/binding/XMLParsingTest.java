...
public class XMLParsingTest extends TestCase {
	...
	public void testParseComposition() throws Exception {
		...
		assertEquals("name wrong", "Blood Glucose", comp.getName().getValue());
		assertEquals("territory wrong", "36", comp.getTerritory().getCodeString());
		
		assertEquals("context startTime wrong", "2006-11-22T18:57:01",
				comp.getContext().getStartTime().getValue());
		
		assertEquals("observation.data.events[0].items[0].data.value.magnitude",
				100.0, ((DVQUANTITY) ((ELEMENT) ((ITEMTREE) ((OBSERVATION) 
						comp.getContentArray(0))
						.getData().getEventsArray(0).getData())
						.getItemsArray(0)).getValue()).getMagnitude());		
	}
