...
public class SpecialisedArchetypeValidator extends ArchetypeValidator {
		...
		if (parentPath != null) {
			...
			if (parentNode == null) {
				...
				if (foundParentFurtherUp)  {
					errors.add(new ValidationError(ErrorType.VSONCI, "The node id "+ccobj.getNodeId()+" does not conform to the parent archetype's node id "+parentNode.getNodeId()+
							" by either being identical, or being a derived identifier at the exact specialisation level of the child."));
				}	
				else if (parentPath != null && parentPath.length() > 0 && (parentPath.endsWith("]") || parentPath.endsWith(ArchetypeConstraint.PATH_SEPARATOR) || Character.isDigit(parentPath.charAt(parentPath.length()-1)))){
					...
					if (StringUtils.countMatches(ccobj.getNodeId(), ".") == childSpecialisationDepth) {
						errors.add (new ValidationError (ErrorType.VATDF, "The path "+parentPath+ " could not be found in the parent, but is specialised to "+ ccobj.path()+"."));
					} else {
						errors.add (new ValidationError (ErrorType.VATDF, "The path "+parentPath+ " could not be found in the parent, but is introduced in the specialised archetype."));
					}	
				} else if (parentPath==null) {				    
				    errors.add (new ValidationError (ErrorType.VATDF, "The path "+parentPathOrig+ " could not be found in the parent, but is introduced in the specialised archetype."));
				}
	...
	private boolean checkDynamicArchetypeType(CObject parentNode,
		...
		if (!ccobj.getClass().getSimpleName().equals(parentNode.getClass().getSimpleName())) {
			errors.add(new ValidationError(ErrorType.VSONT, "The dynamic class "+ccobj.getClass().getSimpleName()+" of the object at " +ccobj.path() +" is different to the dynamic type "+parentNode.getClass().getSimpleName()+" of the corresponding node in the parent archetype."));
			return false;
		}
			return true;
	...
	*/
	...
	private void checkSpecialisedAttributeNodeExistenceConformance(
		...
		|| (cattr.getExistence() == Existence.NOT_ALLOWED &&  
		    ...
		    errors.add(new ValidationError(ErrorType.VSANCE, "Existence " +cattr.getExistence().toString()+ " at " +cattr.path()+ " is not conformant to existence "+attrInParentArchetype.getExistence() +" at "+ attrInParentArchetype.path()+" in the parent archetype."    ));
		}
	}
	...
	private void checkSpecialisedAttributeMultiplicityConformance(
		...
		if (cattr instanceof CMultipleAttribute && !(attrInParentArchetype instanceof CMultipleAttribute)) {
		    errors.add(new ValidationError(ErrorType.VSAM, "Multiplicity (Multiple) of attribute at " +cattr.path()+ " is not conformant to multiplicity (Single) of the attribute at "+ attrInParentArchetype.path()+" in the parent archetype."));
		}	
		// if it is the other way round, this is okay, as the cardinality is contained.
		if (!(cattr instanceof CMultipleAttribute) && (attrInParentArchetype instanceof CMultipleAttribute)) {
		    errors.add(new ValidationError(ErrorType.VSAM, "Multiplicity (Single) of attribute at " +cattr.path()+ " is not conformant to multiplicity (Multiple) of the attribute at "+ attrInParentArchetype.path()+" in the parent archetype."));
		}	
	...
	private void checkSpecialisedAttributeNodeCardinalityConformance(
		...
		if (! cardParentAttr.getInterval().isUpperUnbounded() 
			...
			errors.add(new ValidationError(ErrorType.VSANCC, "Cardinality " +getIntervalFormalString(cardAttr.getInterval())+ " at " +cattr.path()+ " is not conformant to cardinality "+getIntervalFormalString(cardParentAttr.getInterval()) +" at "+ attrInParentArchetype.path()+" in the parent archetype."));
		} else {
			if (cardParentAttr.getInterval().getLower().compareTo(
	...
	private boolean checkSpecialisedNodeIdConformance(CObject parentNode,
		...
		errors.add(new ValidationError(ErrorType.VSONCI, "The node id "+ccobj.getNodeId()+" does not correspond to the parent archetype's node id "+parentNode.getNodeId()+"."));
		return false;
	...
	private void checkNodeIdIsSpecialisedIfRequired(CObject parentNode,
		...
		// If they are different, but have the same node id, this is a VSONIR error.
			// TODO: However, does equals() capture this adequately for this node? 
			errors.add(new ValidationError(ErrorType.VSONIR, "Object node at path " + ccobj.path()+ " (RM type: "+ccobj.getRmTypeName()+ ") redefines corresponding node in parent archetype (RM type: "+parentNode.getRmTypeName()+") but node id "+ccobj.getNodeId()+" is not redefined."));
			...
			if (!archetype.getOntology().termDefinition(langPrim, ccobj.getNodeId()).equals(
				...
				if (parentArchetype.getOntology().termDefinition(langPrim, ccobj.getNodeId()) != null) {
				    errors.add(new ValidationError(ErrorType.VSONIR, "Object node at path " + ccobj.path()+ " (RM type: "+ccobj.getRmTypeName()+ ") redefines parent node (RM type: "+parentNode.getRmTypeName()+") but node id "+ccobj.getNodeId()+" is not redefined. The text/description of the two terms is different."));
				}
			}	
			else {
					...
					for (Entry<String, TranslationDetails> trans : archetype.getTranslations().entrySet()) {
						if (!archetype.getOntology().termDefinition(trans.getValue().getLanguage().getCodeString(), ccobj.getNodeId()).equals(
							...
							if (parentArchetype.getOntology().termDefinition(trans.getValue().getLanguage().getCodeString(), ccobj.getNodeId()) != null) {
							    errors.add(new ValidationError(ErrorType.VSONIR, "Object node at path " + ccobj.path()+ " (RM type: "+ccobj.getRmTypeName()+ ") redefines parent node (RM type: "+parentNode.getRmTypeName()+") but node id "+ccobj.getNodeId()+" is not redefined. The text/description of the two terms is different for language "+trans.getValue().getLanguage().getCodeString()+"."));
							}
						}
					}
				}
	...
	private void checkSpecialisedOccurrencesCompatiblity(CObject parentNode,
		...
		if (! parentNode.getOccurrences().isUpperUnbounded() 
			...
			errors.add(new ValidationError (ErrorType.VSONCO, "Occurrences "+getIntervalFormalString(ccobj.getOccurrences())+" of object at "+ccobj.path()+ " does not conform to occurrences "+getIntervalFormalString(parentNode.getOccurrences())+" of the corresponding parent archetype's node ("+parentNode.path()+".)"));		
		}				
	}
  ...
  private void checkSpecialisedRMTypeCompatiblityForNonValueItems(CObject parentNode,
		...
		if (parentRMType == null || rmChild == null) {
			errors.add (new ValidationError(ErrorType.VSONCT, "RM Type "+ rmChild + " at "+ccobj.path()+" must be identical or conform to the reference model type "+rmParent+" in the corresponding node of the parent archetype. At least one of these RM types is unknown."));
			...
			errors.add (new ValidationError(ErrorType.VSONCT, "RM Type "+ childRMType.getSimpleName() + " of "+ccobj.path()+" must be identical or conform to the reference model type "+parentRMType.getSimpleName()+" in the corresponding node of the parent archetype."));
		}
	}
	...
	private void checkSpecialisedRMTypeCompatiblityOfValueChildren(CAttribute cattr,
		...
		for (CObject childInParent : parentAttr.getChildren()) {
				...
				if (parentRMType == null) {
					errors.add (new ValidationError(ErrorType.VSONCT, "RM Type "+ childInParent.getRmTypeName() + "in parent archetype at "+childInParent.path()+" is unknown. Cannot test properly if the child archetype conforms to this."));
				} else { 
		...
		for (CObject child : children) {
			...
			if (!assignable) {
				...
				if (parentObjectsToRMTypesWithoutGenerics.size() ==1) {
					errors.add (new ValidationError(ErrorType.VSONCT, "RM Type "+ childRMType.getSimpleName() + " of "+child.path()+" must be identical or conform to the reference model type "+parentRefTypes+" in the corresponding node of the parent archetype."));
				} else {
					// more than one possible ones
					errors.add (new ValidationError(ErrorType.VSONCT, "RM Type "+ childRMType.getSimpleName() + " of "+child.path()+" must be identical or conform to at least one of the reference model types "+parentRefTypes+" in the corresponding node of the parent archetype."));
				}
			}
	...
	private void checkTermExistsInParent(Archetype parentArchetype, String code,
		...
		if(priDefs == null) {
			error = new ValidationError(ErrorType.VATDF, 
					"In parent archetype: Missing term definition for " + code);
			errors.add(error);
		} else {
