...
public abstract class Locatable extends RMObject {
    ...
    /**
     * Locate an attribute of type Locatable of given path
     *
     * @param path
     ...
     * @throws IllegalArgumentException if path invalid
     */
    protected Locatable locateAttribute(String path, String[] attributeNames,
                                        Locatable[] attributes) {
        for (int i = 0, j = attributes.length; i < j; i++) {
            Locatable ret = checkAttribute(path, attributeNames[ i ],
                    attributes[ i ]);
            if (ret != null) {
                return ret;
            }
     ...
     */
    protected Locatable checkAttribute(String path, String attributeName,
                                       Locatable attribute) {
        if (attribute == null) {
            return null;
        }
     ...
     * @return null if not matching
     */
    protected Locatable checkAttribute(String path,
